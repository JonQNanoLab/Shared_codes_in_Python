...
for kd in range(0, len(Dplist)):
    Dp = Dplist[kd]
    print("Calculating dipole distribution P[kd, ktheta]")
    print(kd)

    # For each value of the Dp value, we calculate the dipole coupling
    filename = 'Mmatrix' + str(N) + 'spheres' + str(round(Dp/nm, 1)) + 'nm.mat'
    M = sio.loadmat('/home/jon/Desktop/Shared_codes_in_Python/Array_package/\
AvgN50/MmatricesN50/' + filename)['M']
...
sio.savemat('Psupra.mat', {'Psupra': Psupra})
...
for kd in range(0, len(Dplist)):
    print("Calculating fCD average for each (kd, ktheta)")
    print(kd)
